...
class History:
    ...
    def is_win(self):
        if self.is_board_win(self.boards[0]) and self.is_board_win(self.boards[1]):
            return True
        return False
    ...
    def get_valid_actions(self):
        valid_actions = []
        for item in range(9):
            if self.boards[0][item] == '0':
                valid_actions.append(item)
            if self.boards[1][item] == '0':
                valid_actions.append(item + 9)
        return valid_actions

    def is_terminal_history(self):
        if len(self.get_valid_actions()) == 0 or self.is_win():
            return True
        return False
...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    if max_player_flag:
        ...
        for actions in history_obj.get_valid_actions():
            updated_history = History(history=history_obj.history + [actions], num_boards=history_obj.num_boards)
            eval1 = alpha_beta_pruning(updated_history, alpha, beta, False)
        ...
        for actions in history_obj.get_valid_actions():
            updated_history = History(history=history_obj.history + [actions], num_boards=history_obj.num_boards)
            eval2 = alpha_beta_pruning(updated_history, alpha, beta, True)
...
def maxmin(history_obj, max_player_flag):
    ...
    if max_player_flag:
        ...
        for actions in history_obj.get_valid_actions():
            updated_history = History(history=history_obj.history + [actions], num_boards=history_obj.num_boards)
            eval1 = maxmin(updated_history, False)
            max_eval = max(max_eval, eval1)
        board_positions_val_dict[boards_str] = max_eval
        return max_eval
    else:
        min_eval = math.inf
        for actions in history_obj.get_valid_actions():
            updated_history = History(history=history_obj.history + [actions], num_boards=history_obj.num_boards)
            eval2 = maxmin(updated_history, True)
            min_eval = min(min_eval, eval2)
        board_positions_val_dict[boards_str] = min_eval
        return min_eval
